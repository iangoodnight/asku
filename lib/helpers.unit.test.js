...
const { _identifyArgs } = require('./utils.js');
...
test(`_normalizeArgs successful creates arrays, \
  ...
  expect(_normalizeArgs('100-FOO')).toEqual({ ProductSKUs: ['100-FOO']});
	expect(_normalizeArgs('100-FOO', true)).toEqual({ ProductCodes: ['100-FOO']});
	expect(_normalizeArgs('100-FOO', false, true)).toEqual({ ProductSKU: '100-FOO'});		
	expect(_normalizeArgs('100-FOO', true, true)).toEqual({ ProductCode: '100-FOO'});
	expect(_normalizeArgs(['100-FOO','101-FOO']))
	  .toEqual({ ProductSKUs: ['100-FOO','101-FOO']});
	expect(_normalizeArgs(['100-FOO','101-FOO'], false, true))
	  .toEqual({ ProductSKU: '100-FOO'});
	expect(_normalizeArgs(['100-FOO','101-FOO'], true, true))
	  .toEqual({ ProductCode: '100-FOO'});
	expect(_normalizeArgs(['100-FOO','101-FOO'], true))
	  .toEqual({ ProductCodes: ['100-FOO','101-FOO']});
	expect(_normalizeArgs({ skus: ['100-FOO','101-FOO'] }))
	  .toEqual({ ProductSKUs: ['100-FOO','101-FOO']});
	expect(_normalizeArgs({ codes: ['100-FOO','101-FOO'] }))
	  .toEqual({ ProductCodes: ['100-FOO','101-FOO']});
});