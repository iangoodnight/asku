...
const { _curry, _identifyArgs, _propertyTest, _slice } = require('./utils.js');
...
module.exports = {
  ...
  _mixInPageDefaults: function (data, preset = {}) {
    ...
    try {
      ...
      console.log(error); // Add some proper error handling in
    }
  },
  _normalizeArgs: function (args = {}, preferCode = false, string = false) {
    const test = _curry(_propertyTest)(args);
    switch (_identifyArgs(args)) {
      ...
      case 'array':
        if (!preferCode) {
          if (!string) return { ProductSKUs: [...args] };
          if (args.length) return { ProductSKU: args[0] };
        }
        if (!string) return { ProductCodes: [...args] };
        if (args.length) return { ProductCode: args[0] };
        break;
      case 'object':
        if (!preferCode) {
          if (!string) {
          ...
          if (!test('ProductSKUs')) {
            args['ProductSKUs'] = args['skus'] || args['sku'] || '';
          }
          return args;
        }
        if (!string) {
